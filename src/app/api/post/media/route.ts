...
export async function POST(request: Request) {
  try {
    ...
    if (file) {
      ...
      if (exist) {
        console.log(name, "存在");
        const priviewUrl = await client.signatureUrl(name, { process: "image/watermark,text_5YmN56uv6Zuo54i4,resize,w_300,h_300,color_FFFFFF,size_50,shadow_50,g_se,x_40,y_40" });
