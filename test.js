...
![](https://raw.githubusercontent.com/eminoda/myBlog/master/eminoda.github.io/source/_posts/2018-05-06-gumdam-sd-exia/start.png)
周末好累，又要看娃，又要写代码，又要准备考驾照....既然开始撸 blog，就准备开始把玩玩具这个小兴趣扔上面。开设个新类目————高达，我就不信程序员辣么内向没有玩模型的？
希望也能找到同类人，虽然我玩的很菜。
...
function _replaceMdData(mdData, assets) {
  console.log(mdData);
    ...
    if (endIndx !== -1) {
      ...
      for (let { fileName, desc, originUrl } of assets) {
        if (template.indexOf(` ${fileName} ${desc ? desc + ' ' : ''}`) !== -1) {
          ...
          return _replaceMdData(mdData, assets);
        } else {
